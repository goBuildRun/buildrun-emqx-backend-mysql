...
on_message_publish(#message{flags = #{retain := true}} = Message, _Env) ->
    ...
    buildrun_emqx_backend_mysql_cli:query(?MESSAGE_RETAIN_SQL, [binary_to_list(Topic),emqx_guid:to_hexstr(Id),binary_to_list(From),binary_to_list(Qos),binary_to_list(Payload),null,
